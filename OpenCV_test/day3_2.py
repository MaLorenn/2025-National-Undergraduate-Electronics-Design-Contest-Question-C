...
serial_q = queue.Queue(maxsize=3)   # 限制大小，防止阻塞
display_q = queue.Queue(maxsize=3)  # 限制大小，保证实时性
...
class DisplayThread(threading.Thread):
    ...
    def run(self):
        while self.running:
            try:
                img = self.q.get(timeout=1)
                self.frame_count += 1
                if self.frame_count % self.interval == 0:
                    now = time.time()
                    elapsed = now - self.fps_time
                    self.fps = self.interval / elapsed if elapsed > 0 else 0
                    self.fps_time = now
                cv2.putText(img, f"FPS: {self.fps:.1f}", (30, 30),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
                cv2.putText(img, f"Frame: {self.frame_count}", (30, 70),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
                cv2.imshow('Rect True Center Detection', img)
...
# 启动线程
serial_thread = SerialSender(SERIAL_PORT, SERIAL_BAUD, serial_q)
serial_thread.start()
display_thread = DisplayThread(display_q)
...
while display_thread.running:
    ...
    gpu_blur = cv2.cuda.createGaussianFilter(cv2.CV_8UC1, cv2.CV_8UC1, (5, 5), 0).apply(gpu_gray)
    ...
    try:
        ...
        pass
    try:
        display_q.put_nowait(img_raw.copy())
    except queue.Full:
        pass
