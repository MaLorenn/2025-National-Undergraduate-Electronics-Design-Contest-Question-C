    ...
    if hierarchy is not None and len(contours) > 0:
        ...
        for i, contour in enumerate(contours):
            if hierarchy[0][i][3] != -1:
                ...
                if len(approx) == 4:
                    ...
                    if 5500 <= area <= 70000 and area > max_area:
                        max_area = area
        ...
        if approx_inner is not None:
            ...
            if black_ratio > 0.04:
                ...
                for x, y in pts:
                    cv2.circle(img_raw, (int(x), int(y)), 2, (0, 0, 255), 2)
                ...
                if not (np.isnan(cx) or np.isnan(cy)):
                    ...
                    cv2.circle(img_raw, (cx, cy), 2, (0, 0, 255), -1)
                    ...
                    if len(hits) >= 2:
                        ...
                        if len(h_hits) >= 2:
                            ...
                            #更改target坐标
                            offset_v = (91 / 174) * len_v
                            offset_h = (89 / 261) * len_h
                            ...
                            if not (np.isnan(target_pt[0]) or np.isnan(target_pt[1])):
                                ...
                                cv2.circle(img_raw, (tx, ty), 2, (255, 0, 0), -1)
                                cv2.putText(img_raw, "Target", (tx, ty - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2)
                                target_ok = True

    cv2.circle(img_raw, img_center, 2, (0, 255, 0), -1)
