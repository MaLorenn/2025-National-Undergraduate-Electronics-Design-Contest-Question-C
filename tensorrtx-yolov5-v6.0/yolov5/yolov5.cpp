...
int main(int argc, char** argv) {
    ...
    //修改为自己要检测的视频或者图片，注意要写全路径，如果调用摄像头，则括号内的参数设为0，注意引号要去掉。
    ...
    int key;
    int fcount = 0;
    const int avg_N = 20;   // 平滑窗口大小，可调整
    std::deque<double> frame_times_ms;  // 存最近N帧耗时
    ...
    while (1)
    {
        ...
        double elapsed = std::chrono::duration<double, std::milli>(toc - tic).count();
        frame_times_ms.push_back(elapsed);
        if (frame_times_ms.size() > avg_N)
            frame_times_ms.pop_front();

        double avg_time = std::accumulate(frame_times_ms.begin(), frame_times_ms.end(), 0.0) / frame_times_ms.size();
